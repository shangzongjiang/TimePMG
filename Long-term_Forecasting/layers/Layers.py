...
import torch
from .embed1 import DataEmbedding, CustomEmbedding
import math
import numpy as np
...
class Bottleneck_Construct(nn.Module):
    ...
    def __init__(self, d_model, d_inner,top_period):
        super(Bottleneck_Construct, self).__init__()

        if isinstance(top_period, torch.Tensor):
            periods = top_period.view(-1).tolist()  # 转成 [int, int, ...]
        elif isinstance(top_period, list):
            periods = [int(p) if isinstance(p, torch.Tensor) else p for p in top_period]
        else:
            periods = [int(top_period)]

        # 构建卷积层
        self.conv_layers = nn.ModuleList([
            ConvLayer(d_inner, p) for p in periods
        ])

        """
        ##确保top_period是列表
        if isinstance(top_period, list):
            periods = top_period
        else:
            periods = [top_period]
        self.conv_layers = nn.ModuleList()
        for i in range(len(periods)):
            self.conv_layers.append(ConvLayer(d_inner, periods[i].item()))
            # self.conv_layers.append(ConvLayer(d_inner, periods[i]))
                # self.Linear.append(nn.Linear(self.seq_len, self.pred_len))
            # 按周期构建对应数量的 ConvLayer
        self.conv_layers = nn.ModuleList([
            ConvLayer(d_inner, period) for period in periods
        ])

        if not isinstance(top_period, list):
            self.conv_layers = nn.ModuleList([
                ConvLayer(d_inner, top_period),
                ConvLayer(d_inner, top_period),
                ConvLayer(d_inner, top_period)
                ])
        else:
            self.conv_layers = []
            for i in range(len(top_period)):
                self.conv_layers.append(ConvLayer(d_inner, top_period[i]))
            self.conv_layers = nn.ModuleList(self.conv_layers)
            """
        self.up = Linear(d_inner, d_model)####d_inner128 d_model=512
        self.down = Linear(d_model, d_inner)####d_model=512 d_inner128
        self.norm = nn.LayerNorm(d_model)
    # def set_static_freqs_idx(self, top_period):
    ...
    def forward(self, enc_input):####[32,169,512]
        ...
        temp_input=[x.permute(0, 2, 1) for x in enc_input]
        all_inputs = []
        ####对169个节点进行卷积
        for i in range(len(self.conv_layers)):
            temp_out = self.conv_layers[i](temp_input[i])####第一次[32,128,42]第二次[32,128,10]第三次[32,128,2]
            all_inputs.append(temp_out)
        all_output=[x.permute(0, 2, 1) for x in all_inputs]
